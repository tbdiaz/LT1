...
def main():
    ...
    chk("124 nodos totales", len(d["nodes"]) == 124,
        f"total={len(d['nodes'])} (estructural={n_est}, master={n_mas})")
    chk("120 estructurales / 4 masters",
        n_est == 120 and n_mas == 4,
    ...
    chk("6 muros equivalentes", n_mur == 6, str(n_mur))
    chk("12 rigidLink (constraint)", n_rl == 12, str(n_rl))
    chk("18 apoyos", n_apo == 18, str(n_apo))
    chk("4 diafragmas", n_dia == 4, str(n_dia))

    # elementos estructurales = beams + columns + walls (elasticBeamColumn)
    n_ele = n_vig + n_col + n_mur
    chk("204 elasticBeamColumn", n_ele == 204,
    ...
    chk("coordenadas finitas (124 nodos)", coords_finitas)

    # pending_geometry presente
    pend = d.get("pending_geometry")
    chk("pending_geometry presente",
        isinstance(pend, list) and len(pend) >= 8, f"{len(pend) if pend else 0} items")
    ids = [p["id"] for p in (pend or [])]
    for req in ("nucleo_piso_4", "muros_subterraneo_plan101", "V.30/45",
    ...
    chk("analisis OK_COMPLETADO", a.get("estado") == "OK_COMPLETADO",
        str(a.get("estado")))
    chk("P_gravedad ~ 20182.625 kN",
        abs(a.get("P_aplicada_kN", 0) - 20182.625) < 0.01,
        f"P={a.get('P_aplicada_kN')}")
    chk("SumaRz ~ 20182.625 kN",
        abs(a.get("suma_Rz_kN", 0) - 20182.625) < 0.01,
        f"SumaRz={a.get('suma_Rz_kN')}")
    chk("error relativo ~ 9.01e-16",
        abs(a.get("err_rel", 0) - 9.01265022572653e-16) < 1e-17,
    ...
    chk("reacciones 18 / desplazamientos 124",
        len(a.get("reacciones", {})) == 18 and
        len(a.get("desplazamientos", {})) == 124,
